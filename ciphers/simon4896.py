...
def encrypt(p, k, r):
    ...
    for i in range(r):
        ...
        # cipherText_str = hex(x[0]) + hex(y[0])[2:]
        # print(i,"Encrypted Ciphertext: " + cipherText_str)
...
def expand_key(k, t):
    ...
    z = 0b01011010000110010011111011100010101101000011001001111101110001
    for i in range(m, t):
...
def check_testvectors():
  p = np.uint([0x726963 , 0x20646e]).reshape(-1, 1)  
...
#   print(hex(c[0][0]),hex(c[0][1]))
#   print(hex(c[0][0])+hex(c[0][1][2:]))
  assert np.all(c[0] == [0x6e06a5, 0xacf156])
...
'''
[131328, 657672, 1184016, 1710360, 7344579, 12053576, 6780981, 14116675, 2387662, 12197039, 11055477, 15817775, 12434828, 6645492, 9717249, 12979256, 8468155, 838069, 4187764, 12344458, 535153, 14853358, 1054101, 3973077, 9468179, 13520126, 7870214, 3384867, 9675465, 14606834, 6008808, 1631600, 14638791, 13727783, 3669850, 3304247]       
Encrypted Ciphertext: 0x8b82a1726963
'''