...
def check_testvectors():
  ...
  p = np.uint32([0x6d2073, 0x696874]).reshape(-1, 1)
  k = np.uint32([0x1a1918,0x121110,0x0a0908,0x020100]).reshape(-1, 1)
  pb = convert_to_binary(p)
  kb = convert_to_binary(k)
#   print(pb.shape)
#   print(hex(convert_from_binary(pb)[0][0]))

  c = convert_from_binary(encrypt(pb, kb, 23))
  assert np.all(c[0] == [0x735e10, 0xb6445d])
