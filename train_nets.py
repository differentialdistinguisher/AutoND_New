...
def train_one_round(model,
    ...
    """
    if os.path.exists(f'{log_prefix}_{model_name}_training_history_round{round_number}.pkl'):
        history = pd.read_pickle(f'{log_prefix}_{model_name}_training_history_round{round_number}.pkl')
        return np.max(history['val_acc'])
...
def train_neural_distinguisher(starting_round, data_generator, model_name, input_size, word_size, log_prefix = './', _epochs = EPOCHS, _num_samples=None):
    ...
    while True:
        if os.path.exists(f'{log_prefix}_{model_name}_training_history_round{current_round}.pkl'):
            history = pd.read_pickle(f'{log_prefix}_{model_name}_training_history_round{current_round}.pkl')
        ...
        intermediate_results.append({'round': current_round, 'val_acc': val_acc})
    ...
    return best_round, best_val_acc, intermediate_results
