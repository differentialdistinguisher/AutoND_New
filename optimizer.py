...
def current_round_save(log_file,current_round):
    log_file_split = log_file.split("/")
    file_name = "/".join(log_file_split[:len(log_file_split)-1])+"/highest_round.json"
    key = log_file_split[len(log_file_split)-1]
    if not os.path.exists(file_name):
        highest_round = {}
    else:
        with open(file_name, 'r') as f:
            highest_round = json.load(f)
    highest_round[key] = current_round
    with open(file_name, 'w') as file:
        json.dump(highest_round, file)
        

def current_round_get(log_file):
    log_file_split = log_file.split("/")
    file_name = "/".join(log_file_split[:len(log_file_split)-1])+"/highest_round.json"
    key = log_file_split[len(log_file_split)-1]    
    with open(file_name, 'r') as f:
        highest_round = json.load(f)
    return highest_round[key]
...
def optimize(plain_bits, key_bits, encryption_function, nb_samples=NUM_SAMPLES, scenario = "single-key", log_file = None, epsilon=0.1):
    file_best_differences =  f'{log_file}_best_weighted_differences.csv'
    if os.path.exists(file_best_differences):
        diffs_epsilon = np.load(f'{log_file}_diffs_epsilon.npy', allow_pickle=True)
        return(diffs_epsilon, current_round_get(log_file))
    ...
    result, diffs_as_binary, diffs_epsilon = PrettyPrintBestEpsilonCloseDifferences(allDiffs, weightedScores, epsilon, scenario, plain_bits, key_bits)
    np.save(f'{log_file}_diffs_epsilon.npy', diffs_epsilon)
    df, alldiffs = DataframeFromSortedDifferences(allDiffs, weightedScores, scenario, plain_bits, key_bits)    
    df.to_csv(f'{log_file}_best_weighted_differences.csv')
    np.save(f'{log_file}_alldiffs.npy', alldiffs)
    current_round_save(log_file,current_round)
    return(diffs_epsilon, current_round)


def getTopDiffs(log_file,n):
    alldiffs = np.load(f'{log_file}_alldiffs.npy',allow_pickle=True)
    alldiffs = np.flip(alldiffs)
    return alldiffs[:n]
